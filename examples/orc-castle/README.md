...
Этот пример показывает checker на специально подготовленной небольшой Canvas-игре. Это демонстрационный fixture, созданный для наглядного сравнения `before / after`, а не опубликованная production-игра и не результат аудита случайно выбранного проекта.

При этом результаты не нарисованы и не захардкожены: checker действительно запускается для обоих вариантов в browser audit и сам формирует PASS / FAIL / WARN / NOT VERIFIED.

Игра в обоих вариантах сохраняет ту же основную механику. `before` намеренно содержит типовые проблемы интеграции, которые должен обнаружить checker. `after` — та же игра после исправления этих проблем под Яндекс Игры.
